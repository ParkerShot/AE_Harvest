...
3. Выделить слой монеты → **Selected layer → coins**. Оригинал спрячется как `AEH_COIN_SRC`, появятся дубли `AEH_COIN_1..N`. Количество задаётся полем **`Coins per burst`** под кнопкой — оно применяется при каждой сборке.
...
| Coins | монет в залпе — удобнее задавать полем `Coins per burst` в панели |
